...
このDesignDocはn8nワークフローからAWS Bedrock(Claude Sonnet)で動くLambda実装エージェントへ渡されます。Lambda側は次の条件を満たすパッチを生成し、ワークフローがそのパッチをPRブランチにコミットします。
...
- 失敗する品質ゲートがあれば修正したうえで完了とみなす
- 変更内容、使用モデルID、テスト結果、残リスクはワークフローがPRコメントとして投稿する
